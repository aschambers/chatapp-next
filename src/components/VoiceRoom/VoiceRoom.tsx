...
const VoiceRoom = forwardRef<VoiceRoomHandle, Props>(function VoiceRoom(
  ...
  const removePeer = useCallback((targetUsername: string) => {
    ...
    if (pc) {
      pc.close();
      peersRef.current.delete(targetUsername);
    }
  ...
  const createPeer = useCallback(
    (targetUsername: string, stream: MediaStream): RTCPeerConnection => {
  ...
  const avatarSize =
    count === 1 ? 'w-24 h-24 text-3xl' : count <= 4 ? 'w-16 h-16 text-xl' : 'w-12 h-12 text-base';
    ...
    <div className="flex flex-1 flex-col bg-gray-800">
      ...
      <div className="flex items-center justify-between border-b border-gray-600 px-4 py-2 flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="rounded bg-gray-700 px-1.5 py-0.5 text-[0.625rem] text-gray-400">
            Voice
          </span>
        </div>
        {onChatToggle && (
      ...
      <div className="flex flex-1 flex-col items-center justify-center gap-8 p-8">
              ...
              return (
                <div key={user} className="flex flex-col items-center gap-2">
                  <div className="relative">
                    <div
                      className={`flex items-center justify-center rounded-full font-bold text-white select-none ring-2 overflow-hidden ${avatarSize} ${showBadge ? 'ring-red-500 bg-gray-700' : 'ring-gray-600 bg-gray-700'}`}
                    >
                      {userImages?.[user] ? (
                        <img
                          src={userImages[user]!}
                  ...
                  <span className="max-w-[5rem] truncate text-center text-xs text-gray-300">
                    {user}
                  </span>
                </div>
              );
        ...
        {error && <p className="rounded bg-red-900/40 px-3 py-2 text-xs text-red-400">{error}</p>}
      </div>
    </div>
  );
});
