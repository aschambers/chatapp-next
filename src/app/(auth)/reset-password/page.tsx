...
function ResetPasswordContent() {
  ...
  const { isLoading, resetPassSuccess, resetPassError, forgotPassSuccess } = useAppSelector(
    (s) => s.user
  );
    ...
    return (
      <div className="w-full">
        <div className="rounded-lg bg-gray-800 p-8 text-center text-white shadow-lg">
          ...
          <Link href="/login" className="mt-4 block text-yellow-300 hover:underline">
            Back to login
          </Link>
        </div>
      </div>
      ...
      <div className="w-full">
        <div className="mx-auto w-full max-w-sm rounded-lg bg-gray-800 p-8 shadow-lg">
          ...
          <form onSubmit={handleReset} className="space-y-4">
            ...
            {resetPassError && (
              <p className="text-sm text-red-400">Reset failed. Link may have expired.</p>
            )}
            <button
              type="submit"
              disabled={isLoading}
              className="w-full rounded bg-yellow-500 py-2 text-gray-900 hover:bg-yellow-600 disabled:opacity-50"
            >
  ...
  return (
    <div className="w-full">
      <div className="mx-auto w-full max-w-sm rounded-lg bg-gray-800 p-8 shadow-lg">
        ...
        <Link
          href="/login"
          className="mt-4 block text-center text-sm text-gray-400 hover:underline"
        >
          Back to login
        </Link>
      </div>
